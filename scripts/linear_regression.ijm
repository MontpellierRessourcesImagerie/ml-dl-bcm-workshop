...
realW1 = w[0];
realW2 = w[1];
costImage(WIDTH, HEIGHT, 0.5, 0.025, X, Y);
costImageID = getImageID();
...
function costImage(width, height, stepW1, stepW2, X, Y) {
    newImage("cost", "32-bit", width, height, 1);
    imageID = getImageID();
    selectImage(imageID);
...
function createCostPlot(losses) {
